...
import {
  ...
  Check, X, Clock, Phone, Mail, Users, MessageSquare,
} from "lucide-react";
import {
  MOCK_BOOKINGS, PACKAGES, STATUS_STYLES,
  type Booking, type BookingStatus, type PackageId,
} from "../../data";
...
export default function BookingsPage() {
  ...
  const updateStatus = (id: string, next: BookingStatus) =>
  ...
  const filtered = bookings.filter((b) => {
    ...
    const matchesPkg    = pkgFilter === "all"    || b.package === pkgFilter;
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">
            {bookings.length} total bookings · {counts.pending} pending action
          </p>
        </div>
      </div>
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-bold border transition-all duration-200 ${
              statusFilter === s
                ? s === "all"
                  ...
                  : STATUS_STYLES[s as BookingStatus]
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full bg-white/4 border border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-[#a3e635]/40 focus:bg-white/6 transition-all"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            value={pkgFilter}
            onChange={(e) => setPkgFilter(e.target.value as PackageId | "all")}
            className="bg-white/4 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#a3e635]/40 transition-all appearance-none pr-8 cursor-pointer"
          >
            <option value="all">All packages</option>
            {ALL_PACKAGES.map((id) => (
              <option key={id} value={id}>{PACKAGES[id].name}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl border border-white/8 overflow-hidden">
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-neutral-500 text-sm">No bookings match your filters.</p>
          </div>
        ) : (
                ...
                <div key={b.id} className="transition-colors hover:bg-white/[0.015]">
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[10px] font-mono text-neutral-600">{b.id}</span>
                      </div>
                      <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                        <span className="text-xs text-neutral-500" style={{ color: pkg.color }}>
                          {pkg.name}
                        </span>
                        <span className="text-xs text-neutral-600">
                          {format(new Date(b.date), "d MMM yyyy")} · {b.time}
                        </span>
                        <span className="text-xs text-neutral-600 flex items-center gap-1">
                          <Users size={11} /> {b.groupSize}
                        </span>
                      </div>
                    </div>
                    ...
                    {/* Status badge */}
                    <span
                      className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full border flex-shrink-0 ${STATUS_STYLES[b.status]}`}
                    >
                      {b.status}
                    </span>

                    {/* Total */}
                    <span className="text-sm font-black text-white hidden md:block flex-shrink-0">
                      {b.total} SAR
                    </span>
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-5 border-t border-white/5 pt-4 bg-white/[0.02]">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                        <div className="space-y-1">
                          ...
                          <p className="flex items-center gap-2 text-sm text-neutral-300">
                            <Phone size={12} className="text-neutral-500" /> {b.phone}
                          </p>
                          <p className="flex items-center gap-2 text-sm text-neutral-300">
                            <Mail size={12} className="text-neutral-500" /> {b.email}
                          </p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-[10px] font-bold text-neutral-600 uppercase tracking-wider">Session</p>
                          <p className="text-sm text-neutral-300">
                            {format(new Date(b.date), "EEEE, d MMMM yyyy")}
                          </p>
                          <p className="flex items-center gap-2 text-sm text-neutral-300">
                            <Clock size={12} className="text-neutral-500" /> {b.time} · {b.groupSize} players
                          </p>
                        </div>
                        {b.notes && (
                          <div className="space-y-1">
                            ...
                            <p className="flex items-start gap-2 text-sm text-neutral-300">
                              <MessageSquare size={12} className="text-neutral-500 mt-0.5 flex-shrink-0" />
                              {b.notes}
                            </p>
                          </div>
                        )}
                      ...
                      {/* Action buttons */}
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-[10px] font-bold text-neutral-600 uppercase tracking-wider mr-1">
                          Change status:
                        </span>
                        ...
                        <span className="ml-auto text-xs text-neutral-600">
                          Booked {format(new Date(b.createdAt), "d MMM yyyy, HH:mm")}
                        </span>
                      </div>
                    </div>
                  )}
